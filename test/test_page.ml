...
module Leaf = struct
  ...
  let test_allocate_empty =
    Alcotest.test_case "allocate an empty leaf page" `Quick (fun () ->
        ...
        let id, _next_free =
          Aokvs.Page.Allocator.Unsafe.run ~memory_map ~next_free:0
            Aokvs.Page.Leaf.empty
        in

        let page = Aokvs.Page.get_page memory_map id in
  ...
  let test_allocate_singleton =
    Alcotest.test_case "allocate a singleton" `Quick (fun () ->
        ...
        let page = Aokvs.Page.get_page memory_map child.id in
        let leaf = Aokvs.Page.Leaf.of_page page |> Option.get in
  ...
  let test_add =
    Alcotest.test_case "add a record to an existing leaf" `Quick (fun () ->
          ...
          Aokvs.Page.Allocator.(
            Unsafe.run ~memory_map ~next_free:0
              (let* singleton_child =
               ...
               let empty_leaf = page |> Aokvs.Page.Leaf.of_page |> Option.get in
        ...
        let leaf =
          Aokvs.Page.get_page memory_map leaf_child.id
  ...
  let test_add_out_of_order =
    Alcotest.test_case "add records out of order" `Quick (fun () ->
        ...
        let leaf =
          Aokvs.Page.get_page memory_map leaf_id
          |> Aokvs.Page.Leaf.of_page |> Option.get
        in
  ...
  let test_add_with_split =
    Alcotest.test_case "add records and cause a split" `Quick (fun () ->
          ...
          Aokvs.Page.Allocator.(
            Unsafe.run ~memory_map ~next_free:0
              (let* empty_id = Aokvs.Page.Leaf.empty in
               ...
               let* child_split = Aokvs.Page.Leaf.add leaf (test_record 0) in
               let child = child_split |> Either.find_left |> Option.get in
               let* page = get_page child.id in
               let leaf = page |> Aokvs.Page.Leaf.of_page |> Option.get in

               let* child_split = Aokvs.Page.Leaf.add leaf (test_record 1) in
               let child = child_split |> Either.find_left |> Option.get in
               let* page = get_page child.id in
               let leaf = page |> Aokvs.Page.Leaf.of_page |> Option.get in

               let* child_split = Aokvs.Page.Leaf.add leaf (test_record 2) in
               let child = child_split |> Either.find_left |> Option.get in
               let* page = get_page child.id in
               let leaf = page |> Aokvs.Page.Leaf.of_page |> Option.get in
        ...
        let left =
          Aokvs.Page.get_page memory_map left_child.id
          |> Aokvs.Page.Leaf.of_page |> Option.get
        in

        let right =
          Aokvs.Page.get_page memory_map right_child.id
          |> Aokvs.Page.Leaf.of_page |> Option.get
        in
  ...
  let test_make =
    Alcotest.test_case "make a node with two leaves as children" `Quick
      (fun () ->
        ...
        let allocator =
          Aokvs.Page.Allocator.(
            let* left = Aokvs.Page.Leaf.singleton ("KEY_LEFT", "VALUE_LEFT") in
            let* right =
        ...
        let (left, right, node_child), _ =
          Aokvs.Page.Allocator.Unsafe.run ~memory_map ~next_free:0 allocator
        in

        let node =
          Aokvs.Page.get_page memory_map node_child.id
